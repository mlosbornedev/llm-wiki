---
title: SpaceX
created: 2026-04-12
updated: 2026-04-12
type: entity
tags: [company, aerospace, ai, ipo, space]
sources: [raw/transcripts/nate-ipo-spacex-openai-anthropic-2026.md]
---

# SpaceX

Space Exploration Technologies Corp. — aerospace and AI company led by Elon Musk. Planning one of the largest IPOs in history.

## IPO Details
- **Expected valuation:** ~$1.75 trillion
- **Planned raise:** $50–75 billion (would be largest IPO in history, 2x Saudi Aramco 2019)
- **Expected float:** ~3.3% — tiny float strategy (see [[tiny-float-ipo-mechanics]])
- **Exchange:** Nasdaq (reportedly made fast-track index inclusion a condition for choosing Nasdaq over NYSE)
- **Expected timeline:** Within 12 months (as of article date ~April 2026)

## Financials
- Profitable and growing — better financial shape than OpenAI or Anthropic
- Absorbed xAI (Musk's AI company burning ~$1B/month) into SpaceX
- This acquisition added AI moonshot cost burden to SpaceX's balance sheet

## Why the Float is Tiny
See [[tiny-float-ipo-mechanics]]. SpaceX needs to raise $50-75B but doesn't need the market to cough up $450B (which a normal 25% float on $1.75T would require). Selling 3.3% = raising $50-75B while Musk and early investors keep 96.7%.

## Post-IPO Dynamics
- Nasdaq will add to Nasdaq-100 after ~15 trading days (new fast-track rules, see [[nasdaq-fast-track-index-rules]])
- Float weight capped at 5x actual float (~16.5% effective weight despite only 3.3% available)
- Every index fund tracking Nasdaq-100 will be **required to buy** at whatever price the market sets
- See [[index-fund-forced-buying]] for the mechanics of how this flows to 401(k) holders

## Related Concepts
- [[tiny-float-ipo-mechanics]] — why tiny floats make prices reflect scarcity not value
- [[nasdaq-fast-track-index-rules]] — the rule change enabling fast-track index inclusion
- [[index-fund-forced-buying]] — how 401(k) holders are affected
- [[three-ipo-scenarios]] — what could happen next
- [[openai]] — also planning massive IPO
- [[anthropic]] — also planning massive IPO
